...
\begin{equation}
	\pi^*=\argmax_\pi \underset{\tau\sim\pi}{\mathbb{E}}[R^{tot}(\tau)] = \argmax_\pi \underset{{(a_t\sim\pi, s_t\sim P_E)}}{\mathbb{E}}\left[\sum_{t=0}^{T} \gamma^{t}R(s_t,a_t)\right]
\end{equation}
...
\left\{
\begin{split}
	\m{L}_{\pi_\phi} &= \underset{(s\sim P_S)}{\mathbb{E}}\left[Q_\theta(s,\pi_\phi(s))\right] \quad \quad \quad \quad \text{ (Q-Value maximization)}\\
	\m{L}_{Q_\theta} &= \underset{(s\sim P_S,a\sim\pi_c)}{\mathbb{E}}\left[(y- Q_\theta(s,a))^2 \right] \quad \text{(TD-error minimization)}\\
...
A standard way of formalizing the \il problem is to find a policy 
