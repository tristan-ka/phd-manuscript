...
\begin{figure}[!h]
\centering
\includegraphics[width=.8\textwidth]{background/language_acquisition_framework.pdf}	
\caption{The language acquisition framework: A population of agents}
\label{fig:language_acquisition_framework}
\end{figure}


\todo{present the set of objects that we will use,  multi-agent setup without access to internal states of other agents, agents with internal models , population, in out etc.., physical environment (sequential decision making or simpler), collaborative objectiv (refer to section marl)}
...
\begin{algorithm}[!h]
    ...
    \end{algorithmic}
\end{algorithm}
\todo{complete set of interactions as an algorithm}

\todo{say that in this research we will mainly focus on adaptive-self organizing structures}.

\todo{steels takling heads:What is the interaction between cultural evolution and genetic evolution? I have
argued in favour of culture-driven genetic evolution but what kind of genetic change is necessary to support the processes that we have been found to be operating at the cultural level? Clearly, flexible recruitment and huge amounts of memory are key demands on the human brain, but there are undoubtedly others.}

\todo{mention that in the present research we focus on self-organization and the development of agents during a life time and so therefore we ignore all genetic interactions. We moreover consider only populations of two agents!!!!}


\paragraph{Language Games}

\todo{Internal models are tables}
...
\todo{Internal models are neural networks}
